...
sensor_titles = {
    'temp_C': 'Temperature',
    'HR': 'Humidity',
    'windSpeed_m/s': 'Wind speed',
    'windGust_m/s': 'Wind gust',
    'pres_mbar': 'Pressure',
    'solarRad_W/m2': 'Solar radiance',
    'rain_mm/h': 'Rain',
    'rain_day' : 'Rain'
}
...
sensor_units = {
    'temp_C': 'T (ºC)',
    'HR': 'HR (%)',
    'windSpeed_m/s': 'WG (m/s)',
    'windGust_m/s': 'WS (m/s)',
    'pres_mbar': 'p (hPa)',
    'solarRad_W/m2': 'SR (W/m2)',
    'rain_mm/h': 'Rain (mm/h)',
    'rain_day' : 'Rain'
}
...
app.layout = html.Div(children=[
    ...
    html.Div(children='Select year(s):', style={'padding-bottom': '10px'}),
    html.Div([
        dcc.Dropdown(
            id='menu',
    ...
    html.Footer(
        "João Santos | Energy Services | March 2025",
        className='footer'
)

])

# Callback Time Series Power
@app.callback(
...
def update_bar_chart(_):
    bar_data = df.groupby('Year')['Power_kW'].sum().loc[[2017, 2018, 2019]] / 1e6  # Convert to GW
    return {
        'data': [{
